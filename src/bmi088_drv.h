...
public:
     ...
     * @brief 新建一个BMI088驱动
     *
     * @param accel_spidev 加速度计SPI端口，一般为 /dev/spidev0.0
     * @param gyro_spidev 陀螺仪SPI端口，一般为 /dev/spidev0.1
     * @param spi_speed SPI设备速度，不能超过10M
     * @param gpio_chip 中断GPIO设备，一般为 /dev/gpiochip0
     * @param accel_int 加速度计GPIO中断号
     * @param gyro_int 陀螺仪GPIO中断号
     ...
     */
    BMI088Drv(const char* accel_spidev, const char* gyro_spidev, int spi_speed, const char* gpio_chip, uint32_t accel_int, uint32_t gyro_int, AccODR acc_odr, AccBandWidth acc_bw, AccRange acc_range, GyroODR gyro_odr, GyroRange gyro_range);
    ~BMI088Drv();
